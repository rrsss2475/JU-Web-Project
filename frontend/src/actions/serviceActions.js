...
export const serviceDescAction =
  (catName, subCatName, id) => async (dispatch) => {
    try {
      dispatch({ type: PRODUCT_DESC_REQUEST });
      const { data } = await axios.get(
        `/api/services/${catName}/${subCatName}/${id}`
      );
...
export const deleteProduct = (id) => async (dispatch, getState) => {
  try {
    ...
    await axios.delete(`/api/services/admin/${id}`, config);
...
export const listProductDetails = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`/api/services/admin/${id}`, config);
    dispatch({
...
export const updateProduct = (product) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(
      `/api/services/admin/${product._id}`,
      product,
      config
    );
