...
const ProductDescScreen = ({ location }) => {
  ...
  useEffect(async () => {
    axios.get(`http://localhost:5000/api/products/userName/${product.user}`)
  ...
  return (
    <div className='container' style={{ paddingTop: '75px' }}>
      ...
      <Link className='btn btn-dark my-3 mx-2' to={`/categories/${catName}/${subCatName}`}>Back to {subCatName}</Link>
      <Row>
        <Col sm={12} md={8} lg={6} xl={6}>
          <img src={product.image} style={{ height: '100%', width: '100%', maxHeight: "500px" }} />
        </Col>
